...
SELECT * 
FROM BENHNHAN , PHONG
WHERE PHONG.MAPHG= BENHNHAN.MAPHG AND PHONG.LOAIPHONG=N'20 CHỖ';
GO
-- GOM NHÓM 
...
SELECT PG.MAPHG,COUNT (BN.PHAI) AS  'SO BENH NHAN PHAI NAM '
FROM BENHNHAN BN, PHONG PG
WHERE BN.MAPHG=PG.MAPHG AND PG.MAPHG=N'PHG005 ' AND BN.PHAI=N'NAM'
GROUP BY PG.MAPHG
GO
...
SELECT PG.MAPHG,PG.LOAIPHONG ,COUNT (*) AS'SO BENH NHAN'
FROM PHONG PG, BENHNHAN BN 
WHERE BN.MAPHG = PG.MAPHG
GROUP BY PG.MAPHG,PG.LOAIPHONG
HAVING COUNT (*) >= ALL(SELECT  COUNT (*)
FROM PHONG,BENHNHAN
WHERE BENHNHAN.MAPHG = PHONG.MAPHG
